...
if [ -d /sys/devices/system/cpu/cpufreq/policy0 ]; then
	...
	if grep 'schedutil' $AGL; then
		...
		if [ -e $AGL ]; then
			...
			echo 65 > /proc/sys/kernel/sched_spill_load
			echo 1 > /proc/sys/kernel/sched_enable_thread_grouping
			...
			if grep 'blu_active' $AGL; then
				if [ -e $AGL ]; then
					...
					echo 40000 > $LGP/blu_active/timer_rate
					echo 883200 > $LGP/blu_active/hispeed_freq
					echo 0 595200:120000 883200:150000 1401600:175000 > $LGP/blu_active/above_hispeed_delay
					echo 400 > $LGP/blu_active/go_hispeed_load
					echo 0 > $LGP/blu_active/min_sample_time
					echo 1 > $LGP/blu_active/fastlane
					echo 60 > $LGP/blu_active/fastlane_threshold
...
if [ -d /sys/devices/system/cpu/cpufreq/policy4 ]; then
	...
	if grep 'schedutil' $AGB; then
		if [ -e $AGB ]; then
			...
			echo 4000 > $BGP/schedutil/down_rate_limit_us
		fi
		echo "	+Tuning finished for schedutil" >> $DLL
		...
		if [ -e $AGB ]; then
			...
			echo 74 576000:75 1132800:85 1651200:86 1881600:91 2265600:93 > $BGP/blu_active/target_loads
			chmod 444 /sys/devices/system/cpu/cpu4/cpufreq/blu_active/target_loads
			echo 178470 > $BGP/blu_active/timer_slack
			echo 1574400 > $BGP/blu_active/hispeed_freq
			chmod 644 $BGP/blu_active/timer_rate
			echo 40000 > $BGP/blu_active/timer_rate
			echo 50000 1132800:200000 1651200:175000 1881600:80000 > $BGP/blu_active/above_hispeed_delay
			echo 400 > $BGP/blu_active/go_hispeed_load
			echo 25000 > $BGP/blu_active/min_sample_time
			echo 0 > $BGP/blu_active/align_windows
			echo 1 > $BGP/blu_active/fastlane
			echo 70 > $BGP/blu_active/fastlane_threshold
		...
		if [ -e $AGB ]; then
			...
			echo 74 576000:75 1132800:85 1651200:86 1881600:91 2265600:93 > $BGP/interactive/target_loads
			...
			echo 50000 1132800:200000 1651200:175000 1881600:80000 > $BGP/interactive/above_hispeed_delay
